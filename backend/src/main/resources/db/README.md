...
Generated on 2026-08-22 from the running `songtexts-db-dev` Postgres container:
...
At generation time the database had no standalone non-constraint indexes; primary-key and unique indexes are represented by the corresponding constraints in the SQL dump.
